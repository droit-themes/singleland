...
function singleland_body_classes( $classes ) {
...
add_filter( 'body_class', 'singleland_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */

function singleland_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'singleland_pingback_header' );

function singleland_continue_reading_text() {
	$continue_reading = sprintf(
		/* translators: %s: Name of current post. */
		esc_html__( 'Continue reading %s', 'singleland' ),
...
function singleland_continue_reading_link_excerpt() {
	global $post;
	return ' ...';
}

// Filter the excerpt more link.
add_filter( 'excerpt_more', 'singleland_continue_reading_link_excerpt' );
...
function singleland_print_first_instance_of_block( $block_name, $content = null, $instances = 1 ) {
	$instances_count = 0;
	$blocks_content  = '';
...
if(!function_exists('singleland_can_show_post_thumbnail')) : 
/**
 ...
 * @return bool
 */
function singleland_can_show_post_thumbnail() {
	return apply_filters(
		'singleland_can_show_post_thumbnail',
 ...
 *  
 **/
add_filter( 'get_custom_logo', 'singleland_custom_logo_class' );


function singleland_custom_logo_class( $html ) {
...
//Comment Field Order
add_filter( 'comment_form_fields', 'singleland_comment_fields_custom_order' );
function singleland_comment_fields_custom_order( $fields ) {
...
if(!function_exists('singleland_wrapper_start')) {

	function singleland_wrapper_start( $sidebar = 'right', $column = 8 ) {
...
if(!function_exists('singleland_wrapper_end')) {

	function singleland_wrapper_end( $sidebar = 'right' ) {
		?>
		  </div>
		 <?php
			if($sidebar != 'full' && is_active_sidebar( 'blog_sidebar' )){
...
if(!function_exists('singleland_pagination')){

	function singleland_pagination(){
...
if ( !function_exists('singleland_header_footer_builder_id')){

    function singleland_header_footer_builder_id($posttype, $post_id) {
